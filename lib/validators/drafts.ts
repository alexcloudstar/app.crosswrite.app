import { z } from 'zod';
import { UUID, StringMax, validatePayloadSize, sanitizeContent, sanitizeTitle, sanitizeTags } from './common';

const draftBase = z.object({
  title: z.string().min(1, 'Title is required').max(200, 'Title too long').transform(sanitizeTitle),
  content: z.string().min(1, 'Content is required').max(15000, 'Content too long').transform(sanitizeContent),
  contentPreview: z.string().max(500, 'Preview too long').optional().transform(val => val ? sanitizeContent(val) : val),
  platforms: z.array(z.string()).default([]),
  thumbnailUrl: z.string().url('Invalid thumbnail URL').optional().or(z.literal('')),
  seoTitle: z.string().max(60, 'SEO title too long').optional().transform(val => val ? sanitizeTitle(val) : val),
  seoDescription: z.string().max(160, 'SEO description too long').optional().transform(val => val ? sanitizeContent(val) : val),
