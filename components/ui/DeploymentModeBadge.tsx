...
export function DeploymentModeBadge({ mode, hasRequiredKey }: DeploymentModeBadgeProps) {
  if (mode === 'HOSTED') {
    return (
      <div className="flex items-center space-x-2">
        <div className="badge badge-info badge-sm">HOSTED</div>
        {hasRequiredKey ? (
          <span className="text-sm text-success">Powered by Cross Write key</span>
        ) : (
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      {hasRequiredKey ? (
        ...
        <span className="text-sm text-error">Set OPENAI_API_KEY in environment</span>
      )}
    </div>
  );
}
